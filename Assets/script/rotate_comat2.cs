...
public class rotate_comat2 : MonoBehaviour {
	...
	void Update() {
		...
		if (Input.touchCount ==2) {
			...
			if (Input.GetTouch (1).phase == TouchPhase.Moved) {
				...
				if (xAngle > 270 && first.scene == 1) {
					xAngle = 270f;

				}else if (xAngle < 190 && first.scene==1) {
					xAngle = 190f;

				}

				if (xAngle > 145 && first.scene==2) {
					xAngle = 145f;

				}else if (xAngle < 60 && first.scene==2) {
					xAngle = 60f;
				...
				transform.localRotation = Quaternion.Euler (yAngle, 0.0f, 0.0f);
